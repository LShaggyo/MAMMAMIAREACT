...
export default function Home() {
    ...
    const handleAddToCart = (pizzaId) => {
        addToCart(pizzaId);
        setTotalPrice(setPizzaArray(pizzaId));
        setCartUpdated(true); 
    };
    ...
    return (
        <div>
            <header className="header">
                ...
                <h4>Tenemos las mejores pizzas que podrías encontrar</h4>
            </header>
            <Container>
                <CardGroup className="m-1 text-align-center">
                    {pizzas.map((pizza) => (
                        <Card className='m-1' style={{ width: '18rem' }} key={pizza.id}>
                            ...
                            <Card.Body>
                                ...
                                {pizza.ingredients.map((ingredient) => (
                                    <ListGroup.Item className='text-capitalize'>🍕 {ingredient}</ListGroup.Item>
                                ))}
